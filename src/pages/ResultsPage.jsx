...
const resultBadge = (result) => {
  ...
  switch (result) {
    case 'HIT':  return { ...base, color: mlbDark.hitText, background: mlbDark.hitBg };
    case 'MISS': return { ...base, color: mlbDark.missText, background: mlbDark.missBg };
    case 'PUSH': return { ...base, color: mlbDark.pushText, background: mlbDark.pushBg, fontWeight: 600 };
    default:     return { ...base, color: dark.textSecondary, background: dark.surfaceBg, fontWeight: 600 };
  }
};
...
const thStyle = {
  padding: '10px 10px', color: dark.textPrimary, fontSize: '12px',
...
export default function ResultsPage() {
  ...
  if (loading) return <div style={{ textAlign: 'center', padding: '60px', color: dark.textMuted }}>Loading results...</div>;
  ...
  return (
    <div style={{ maxWidth: 880, margin: '0 auto', padding: '24px 16px 60px' }}>
      ...
      <h1 style={{ color: dark.textPrimary, fontSize: '22px', fontWeight: 800, margin: '0 0 16px', textAlign: 'center' }}>
        Daily Results
      </h1>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '12px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <button onClick={() => prevDate && setSelectedDate(prevDate)} disabled={!prevDate}
          style={{ background: prevDate ? dark.pageBg : mlbDark.disabledBg, color: prevDate ? dark.textPrimary : dark.textSecondary,
            border: 'none', borderRadius: '6px', padding: '8px 14px', fontWeight: 700, fontSize: '13px',
            cursor: prevDate ? 'pointer' : 'default' }}>
          Prev
        </button>
        <select value={selectedDate || ''} onChange={(e) => setSelectedDate(e.target.value)}
          style={{ background: dark.inputBg, color: dark.textPrimary, border: `2px solid ${dark.border}`,
        ...
        <button onClick={() => nextDate && setSelectedDate(nextDate)} disabled={!nextDate}
          style={{ background: nextDate ? dark.pageBg : mlbDark.disabledBg, color: nextDate ? dark.textPrimary : dark.textSecondary,
            border: 'none', borderRadius: '6px', padding: '8px 14px', fontWeight: 700, fontSize: '13px',
      ...
      {/* Day content */}
      {!day ? (
        <div style={{ textAlign: 'center', background: dark.surfaceBg, borderRadius: '10px', padding: '40px 20px',
          color: dark.textSecondary, fontSize: '15px' }}>
          No published picks this date.
        </div>
      ) : (
        <>
          {/* Day record header */}
          <div style={{ textAlign: 'center', background: dark.pageBg, borderRadius: '14px', padding: '20px', marginBottom: '12px' }}>
            <div style={{ color: dark.statusGreen, fontSize: '42px', fontWeight: 800, lineHeight: 1 }}>{pct(day.rate)}</div>
            <div style={{ color: dark.textPrimary, fontSize: '14px', fontWeight: 600, marginTop: '6px' }}>
              {day.hits} hits, {day.scored - day.hits} misses ({day.scored} graded)
            </div>
            <div style={{ color: dark.textSecondary, fontSize: '12px', marginTop: '2px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '20px' }}>
            {(day.blocks || []).map((block) => (
              <a key={block.card_type} href={`#block-${block.card_type}`}
                style={{ background: dark.surfaceBg, border: `1px solid ${dark.border}`, borderRadius: '6px',
                  padding: '6px 12px', fontSize: '12px', fontWeight: 600, color: dark.textPrimary,
          ...
          {(day.blocks || []).map((block) => (
            <section key={block.card_type} id={`block-${block.card_type}`} style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between',
                padding: '10px 12px', background: dark.surfaceBg, borderRadius: '10px 10px 0 0' }}>
                <div>
                  <span style={{ color: dark.textPrimary, fontSize: '15px', fontWeight: 700 }}>
                    {BLOCK_LABELS[block.card_type] || block.type}
                  </span>
                  {block.direction && (
                    <span style={{ color: dark.textSecondary, fontSize: '12px', marginLeft: '8px' }}>
                      {block.direction}
                    </span>
                  )}
                </div>
                <span style={{ color: dark.textSecondary, fontSize: '13px' }}>
                  ...
                  <tbody>
                    {block.picks.map((p, i) => (
                      <tr key={i} style={{ borderTop: `1px solid ${dark.border}`, background: i % 2 ? mlbDark.rowStripe : dark.surfaceBg,
                        ...
                        <td style={{ padding: '8px 10px', fontWeight: 600, color: dark.textPrimary, fontSize: '13px' }}>
                          {p.player}
                        </td>
                        <td style={{ padding: '8px 10px', fontSize: '12px', color: dark.textSecondary, textAlign: 'left' }}>
                          {p.matchup}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: dark.textSecondary, fontVariantNumeric: 'tabular-nums', fontSize: '13px' }}>
                          {fmtNum(p.projected)}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 600, fontVariantNumeric: 'tabular-nums', fontSize: '13px', color: dark.textPrimary }}>
          ...
          {/* Exclusions note */}
          <p style={{ textAlign: 'center', color: dark.textSecondary, fontSize: '12px', margin: '8px 0 0' }}>
            Strikeouts, Home Runs (HR Watch), Game Lines, and the Parlay Builder are not part of the graded record.{' '}
            <Link to="/mlb/track-record#methodology" style={{ color: dark.textSecondary }}>See methodology</Link>.
          </p>
        </>
      )}
    </div>
